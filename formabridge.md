...
DB에 쌓인 곡 레코드 중 상당수가 `spotifyId` 없이 제목·아티스트만 가지고 있었습니다. Spotify 연동 기능을 붙이려면 기존 레코드에 이 키를 채워 넣어야 했고, 건당 외부 API 호출이 필요해 서비스 요청 흐름 안에서 처리할 수 없었습니다. 별도 배치 스크립트로 분리했습니다.
...
동명이곡이나 커버 버전이 상위에 오면 엉뚱한 ID가 박히고, 한국어 제목은 특히 매칭이 불안정합니다. 그래서 **쓰기 전에 무엇이 바뀔지 먼저 눈으로 확인할 수 있게** 만들었습니다.
