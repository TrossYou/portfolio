...
export const projects: Project[] = [
  {
    slug: 'finch',
    name: 'FINCH',
    tagline: 'AI 투자 비서를 붙인 증권 서비스 · 모바일 웹',
    year: '2026',
    period: '2026.08– · 진행 중',
    team: '5인',
    role: '프론트엔드',
    ...
    accent: '#5b4bc4',
    summary:
      '팀 5인 중 프론트엔드 구현 전반을 맡고 있다. 토스증권을 UI 레퍼런스로 삼아 증권 서비스를 클론하고 그 위에 AI 투자 비서를 얹는다. 화면보다 먼저 인증 경로와 API 계약을 어떻게 받을지부터 정했다.',
    metrics: [
      { value: '1벌', label: '프론트 HTTP 클라이언트' },
      { value: '3개', label: '세션을 건드리는 에러 코드' },
    ...
    chapters: [
      {
        ...
        blocks: [
          {
            kind: 'lead',
            body: '팀 5인 중 프론트엔드 구현 전반을 맡고 있습니다. 모바일 웹 증권 서비스에 투자 판단을 돕는 AI 비서를 붙이는 프로젝트입니다.',
          },
          {
            kind: 'text',
            body: '아직 진행 중이라 완성된 화면이나 운영 결과는 없습니다. 이 글은 구현 결과보다 구현에 들어가기 전에 내린 판단을 중심으로 적습니다.',
          },
        ],
      },
      {
      ...
      {
        no: '03',
        title: '"인증 로직이 두 벌"을 판단할 수 없었다',
        blocks: [
          {
            kind: 'text',
            body: 'AI 파트에서 "프론트가 AI 서버를 직접 호출하면 AI 서버에도 JWT 인증이 필요해져 인증 로직이 두 벌이 된다"는 의견이 나왔습니다. 무엇이 두 벌이 된다는 건지 구분할 수 없었고, 그래서 프론트 HTTP 클라이언트를 몇 벌 둘지도 판단할 수 없었습니다.',
          },
          {
            kind: 'text',
            body: '튜터 세션으로 JWT의 payload·서명·검증을 하나씩 확인했습니다. payload는 감춰지는 게 아니라 누구나 디코드할 수 있고, 서명은 내용과 비밀 키가 둘 다 있어야 계산됩니다. 여기까지 오니 **두 벌이 되는 것은 코드가 아니라 검증에 쓰는 비밀 키가 놓일 자리**라는 게 보였습니다.',
          },
          {
            kind: 'text',
            body: '호출 경로는 팀 논의에서 백엔드 경유로 정해졌습니다. 사용자 토큰을 검증하는 곳이 백엔드 한 곳이므로, 프론트는 HTTP 클라이언트를 한 벌로 두고 AI 전용 클라이언트나 응답 변환 계층을 따로 만들지 않기로 했습니다.',
          },
        ],
      },
      {
        no: '04',
        title: '401을 세션 만료로 보지 않았다',
        blocks: [
          {
            kind: 'text',
            body: '명세상 백엔드는 AI 서버의 에러를 상태 코드째 그대로 통과시킵니다. 백엔드와 AI 서버 사이의 내부 인증이 어긋나도 401이 프론트까지 내려온다는 뜻입니다. 인터셉터가 숫자만 보고 분기하면 **세션이 멀쩡한데 AI 탭을 눌렀다가 로그아웃**되는 일이 생깁니다. 이 문제는 백엔드가 명세 리뷰에서 먼저 짚었습니다.',
          },
          {
            kind: 'text',
            body: 'AI 경로를 인터셉터 밖으로 빼는 안은 기각했습니다. 클라이언트가 두 갈래가 되고, 경로 목록 갱신을 잊으면 방어가 조용히 뚫립니다. 대신 방향을 뒤집어 **아는 인증 코드 세 개일 때만 세션을 건드리는 화이트리스트**로 정해졌습니다. 모르는 코드가 오면 아무 일도 일어나지 않습니다.',
          },
          {
            kind: 'quote',
            body: '위험한 것의 목록은 낡으면 뚫리지만, 안전한 것의 목록은 우리가 통제합니다.',
          },
          {
            kind: 'text',
            body: '인터셉터 코드가 생기기 전에 정한 설계 원칙입니다. 이 원칙은 인증 코드 이름공간이 겹치지 않아야 성립해서, 그 합의를 다른 파트에 이슈로 요청했습니다.',
          },
        ],
      },
      {
