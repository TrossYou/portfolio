...
export const projects: Project[] = [
  {
    ...
    accent: '#5b4bc4',
    summary:
      '5인 팀에서 프론트엔드 구현 전반을 맡고 있습니다. 실시간 시세 구독, 기간별 캔들 차트, 수량 기반 시장가 매수·매도와 예수금 화면을 구현했습니다. 백엔드·AI 파트와의 API 계약 정리와 목 서버 관리도 맡고 있습니다.',
    metrics: [
      { value: '1벌', label: '프론트 HTTP 클라이언트' },
      { value: '3개', label: '설계에서 정한 세션 처리 코드' },
      { value: '2곳', label: '계약이 틀렸을 때 고친 곳' },
    ],
    stack: ['React', 'TypeScript', 'Vite', 'TanStack Query', 'Zustand', 'Zod', 'MSW'],
    links: [],
    note: '진행 중인 프로젝트입니다. 서비스와 화면은 아직 완성되지 않았습니다.',
    chapters: [
      {
        no: '01',
        title: '서비스와 구현 범위',
        blocks: [
          {
            kind: 'lead',
            body: '내 계좌와 성향을 읽고 투자 판단을 돕는 AI 비서가 있는 증권 앱입니다. 5인 팀에서 프론트엔드 구현 전반을 맡고 있습니다.',
          },
          {
            kind: 'list',
            items: [
              '**실시간 시세** — 백엔드가 중계하는 시세를 화면 단위로 구독·해제하는 기능을 구현했습니다.',
              '**기간별 캔들 차트** — 가격의 움직임을 기간별로 보여주는 차트를 구현했습니다.',
              '**주문·예수금** — 수량을 입력해 시장가로 매수·매도하는 주문 화면과 예수금 화면을 구현했습니다.',
              '**화면** — 종목 상세·홈·AI 채팅·예수금·주문·거래내역·문의함을 맡고 있습니다.',
              '**개발 기반** — 백엔드·AI 파트와의 API 계약 정리, 서버 응답을 대신하는 목 서버(MSW), 프론트엔드 개발 규약을 맡고 있습니다.',
            ],
          },
          {
            kind: 'text',
            body: '현재 개발 진행 중입니다. 다른 프론트엔드 팀원은 포트폴리오·인증 화면과 디자인 문서를 맡고 있습니다.',
          },
        ],
      },
      {
        no: '02',
        title: '새 서비스 대신 클론을 골랐다',
        blocks: [
          {
            kind: 'text',
            body: '주제는 핀테크로 주어져 있었고, 그 안에서 무엇을 만들지는 정해야 했습니다. 시장에 없는 새 서비스를 만드는 방향과 **기존 증권 앱을 클론하고 AI를 얹는 방향** 중 뒤쪽을 골랐습니다.',
          },
          {
            kind: 'text',
            body: '새 서비스는 만들어도 비교할 대상이 없어 잘 만들었는지 판단하기 어렵습니다. 이미 있는 서비스를 기준으로 삼으면 완성도의 잣대가 밖에 있고, 그 위에 무엇을 더했는지가 분명해집니다.',
          },
        ],
      },
      {
        no: '03',
        title: '모르는 개념을 공부해 호출 경로의 판단 재료 정리',
        blocks: [
          {
            kind: 'text',
            body: 'AI 채팅 화면을 만들기 위해 다른 파트에 AI 서버를 직접 호출할지, 백엔드를 거칠지 문의했습니다. 직접 호출하면 AI 서버에서도 사용자의 인증 정보를 검증해야 한다는 회신을 받았지만 그 부담을 판단할 만큼 개념을 알지 못한 상태였습니다.',
          },
          {
            kind: 'text',
            body: '사용자의 인증 정보를 담는 토큰인 JWT부터 학습했습니다. payload는 누구나 읽을 수 있는 내용이고, 서명은 내용의 위조 여부를 확인하는 값이라는 점을 구분했습니다. 당시 검토한 HS256 방식에서는 토큰 발급과 검증에 같은 비밀 키를 쓰므로 AI 서버가 직접 검증하려면 그 서버에도 인증 키가 필요하다는 점을 확인했습니다.',
          },
          {
            kind: 'table',
            head: ['호출 경로', '결정하는 파트에 전달한 장단점'],
            rows: [
              ['프론트엔드가 AI 서버를 직접 호출', '백엔드를 거치지 않는 대신 AI 서버에도 인증 키가 필요'],
              ['백엔드를 거쳐 AI 서버 호출', '사용자 인증 키를 백엔드에 두는 대신, 당시 계획한 SSE 답변 전송에서 중계 병목이 생길 가능성'],
            ],
          },
          {
            kind: 'text',
            body: 'SSE는 서버가 답변을 여러 조각으로 나누어 보내는 방식입니다. 설계 단계에서 예상한 중계 병목과 인증 키 관리 부담을 비교해 다른 파트에 전달했습니다.',
          },
          {
            kind: 'text',
            body: '다른 파트는 인증 키 관리 부담을 더 크게 보고 백엔드 경유와 SSE 폐기를 결정했습니다. 이후 팀 API 명세에 반영된 경로를 확인하고 프론트엔드 HTTP 클라이언트를 하나로 두는 구성을 정리했습니다. AI 응답 형식의 변환은 백엔드에서 처리합니다.',
          },
        ],
      },
      {
        no: '04',
        title: '구현 전에 정한 세션 처리 기준',
        blocks: [
          {
            kind: 'text',
            body: 'AI 분석 화면의 요청이 실패했을 때 백엔드와 AI 서버 사이의 인증 오류도 HTTP 상태 401로 프론트엔드에 전달되는 명세입니다. 백엔드는 명세 리뷰에서 이를 사용자 로그인 만료로 잘못 처리할 위험을 먼저 지적했습니다.',
          },
          {
            kind: 'text',
            body: '이 지적을 전체 요청의 응답을 확인하는 인터셉터 설계로 확장했습니다. AI 요청만 인터셉터에서 제외하는 안은 공통 처리 코드가 중복되고 경로 목록을 계속 관리해야 하므로 기각했습니다.',
          },
          {
            kind: 'text',
            body: '인터셉터 구현 전에 허용한 코드만 세션 처리로 보내는 화이트리스트 원칙을 제안했습니다. AUTH_TOKEN_EXPIRED·AUTH_INVALID_TOKEN·AUTH_REFRESH_TOKEN_MISSING 세 코드일 때만 사용자 세션을 처리합니다. 나머지 오류는 세션을 변경하지 않고 요청한 쪽으로 전달하는 기준입니다.',
          },
          {
            kind: 'text',
            body: '이 원칙이 성립하려면 사용자 인증 코드와 AI 서버의 오류 코드가 겹치지 않아야 하므로 코드 이름을 나누어 쓰는 합의를 다른 파트에 이슈로 요청했습니다.',
          },
        ],
      },
      {
        no: '05',
        title: '정해지지 않은 계약을 추정으로 적었다',
        blocks: [
          {
            kind: 'text',
            body: '처음에는 다른 파트가 정하지 않은 API 계약의 회신을 기다렸고, 그 기다림이 그대로 개발의 병목이 됐습니다. 그래서 **정해지지 않은 부분은 추정으로 적고 먼저 개발한 뒤, 확정되면 고치는 쪽**으로 바꿨습니다. 계약은 확정·미확정·충돌로 나눈 원장 문서에 적어 둡니다.',
          },
          {
            kind: 'text',
            body: 'AI 채팅 생성이 비동기 작업으로 바뀌었을 때 새 경로가 아직 확정되지 않아 잠정 확정으로 받고 진행했습니다. 같은 날 응답 모양이 확정됐고 프론트 추정은 한 곳에서 틀렸습니다. 고친 곳은 어댑터와 목 서버 둘이었고, 이 건에서 화면 코드는 바뀌지 않았습니다.',
          },
          {
            kind: 'text',
            body: '대가도 있었습니다. 원장을 다시 짚어 보니 이미 확정됐는데 표만 따라가지 못한 항목이 섞여 있었습니다. 적어 두고 먼저 가는 방식은 원장을 계속 따라 고쳐야 값을 합니다.',
          },
        ],
      },
    ],
...

  {
    ...
    metrics: [
      { value: '약 806ms 전', label: '변경 후 서체 준비 · 검색 결과 렌더 기준' },
    ...
    chapters: [
      {
        ...
        blocks: [
          {
            kind: 'lead',
            body: '6인 팀에서 프론트엔드 기능 구현을 맡았습니다. 프론트엔드 저장소 커밋 206건 중 178건을 담당했습니다.',
          },
          {
            kind: 'list',
            items: [
      ...
      },
      {
        ...
        blocks: [
          {
            kind: 'text',
            body: '**검색 결과 서체의 준비 시점 검증.** 검색 결과가 표시된 뒤 서체가 바뀌었습니다. 검색창에 포커스가 들어올 때 서체를 미리 받는 개선을 제안하고 수치로 검증했습니다. 서체 내려받기 시작은 페이지 진입 후 913ms에서 149ms로 변경됐습니다. 서체 준비 시점은 검색 결과 렌더 약 11ms 뒤에서 약 806ms 전으로 변경됐습니다.',
          },
          {
            kind: 'text',
      ...
      {
        no: '03',
        title: '병렬 작업의 실패와 파일 기준 재설계',
        blocks: [
          { kind: 'text', body: '직렬로 지시하고 결과를 옮기는 대기를 줄이려고 독립된 작업 폴더를 만드는 `git worktree`로 병렬 작업 레인 3개를 구성했습니다. 레인은 에이전트가 작업을 진행하는 단위입니다.' },
          {
            kind: 'list',
            items: [
          ...
          { kind: 'text', body: '레인을 나누는 기준을 폴더에서 실제로 수정할 파일 목록인 **파일 footprint**로 변경했습니다. 티켓별 대상 파일을 먼저 정하고 파일이 겹치는 티켓은 같은 레인에 순차 배정했습니다. 겹치지 않는 티켓만 병렬 배정하되, 한쪽 작업이 다른 쪽의 임계값에 영향을 주면 실행 순서를 고정했습니다.' },
          { kind: 'text', body: 'PR #125·#126·#127의 파일 목록을 대조해 교집합이 0임을 확인했습니다.' },
          { kind: 'text', body: '**본인 회고** — 병렬 작업의 결과를 검토하는 일이 다음 작업의 대기로 이어졌다는 판단입니다. 한 레인을 검토하는 동안 다른 레인은 대기했고 레인마다 개발 서버를 따로 띄워야 했습니다.' },
          { kind: 'text', body: '실패 사례는 `docs/troubleshooting/`에 기록했습니다. 2026-08-07 기준 15건입니다.' },
        ],
      },
      {
        no: '04',
    ...
    chapters: [
      {
        ...
        blocks: [
          ...
          {
            kind: 'text',
            body: '여기서 만진 세션 방식이 나중에 formabridge에서 JWT로 옮겨갈 때 무엇이 달라지는지 이해하는 바탕이 됐습니다. 세션은 서버가 로그인 상태를 들고 있고, JWT는 토큰 자체가 그 정보를 들고 다닙니다.',
          },
        ],
      },
      {
