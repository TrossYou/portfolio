    ...
    chapters: [
      ...
      },
      {
        ...
        blocks: [
          ...
          {
            kind: 'text',
            body: '곡 제목과 아티스트로 Spotify를 검색해 첫 번째 결과의 키를 가져오는 방식인데, **맞는지 틀렸는지가 로그만 봐서는 드러나지 않습니다.** 동명이곡이나 커버가 위에 오면 엉뚱한 키가 들어가고, 그 상태로 화면에 나가면 다른 곡의 플레이어가 뜹니다.',
          },
          {
            kind: 'text',
            body: '그래서 쓰기 전에 무엇이 바뀔지 먼저 찍어보는 모드를 만들었습니다. 곡 하나하나에 대해 어떤 키가 들어갈지 출력만 하고 DB는 건드리지 않습니다.',
          },
          {
            kind: 'code',
            lang: 'text',
...
[DRY] would update songId=92 spotifyId=7rPEpMvt602Np1i7TFk3Hc`,
          ...
          {
            kind: 'text',
            body: '기본값을 이 모드 쪽에 뒀습니다. 설정을 빠뜨린 채로 실행하면 데이터가 바뀌지 않는 쪽으로 실패합니다.',
          },
        ],
      },
      {
